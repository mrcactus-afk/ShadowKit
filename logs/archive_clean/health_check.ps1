...
Write-Output "[$timestamp] WATCHDOG_PROCESS: $($watchdogRunning ? 'RUNNING' : 'NOT_RUNNING')"
...
$reportContent = Get-Content -Path $reportPath -ErrorAction SilentlyContinue
$reportContent += @(
    "[$timestamp] WATCHDOG_PROCESS: $($watchdogRunning ? 'RUNNING' : 'NOT_RUNNING')",
